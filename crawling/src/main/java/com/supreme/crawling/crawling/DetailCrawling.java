...
public class DetailCrawling {
    public static void main(String[] args) {
        ...
        String sql = "";
        int x = 0;
        String path = "/Users/oyun-yeong/Desktop/Shoekream/pro_detail_img/"; // 파일저장위치

        for (int i = 0; i <= 10; i++) {
            url = kreamurl + (30031 + i);
            try {
            ...
            Elements imgs = doc.getElementsByAttributeValue("class", "item_inner"); // 상품사진
            ...
            Elements p_sizes = doc.select("div.size");  // 사이즈
            ...
            while(p_brand.hasNext()) {
                ...
                try {
                    ...
                    sql = "insert into pro_detail(img, brand, name, name_kor, size, recent_price, fluctuation, now_buy, now_sell, wish_count, model_num, release_date, color, first_price) " +
                            "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    System.out.println("🟡");
                    ...
                    pstmt.setString(11,model_num);
                    pstmt.setString(12,release_date);
                    pstmt.setString(13,color);
                    pstmt.setString(14,first_price);
